...
OPENAPI_TAG_METADATA = {"name": school_models.MODEL_TYPE, "description": "Schools data"}
...
async def find_schools(request: Request, organizational_unit: Optional[str] = None):
    ...
    return school_models.ListAPISchoolResponse(data=data, links={"self": unquote(self_url)})
...
async def get_school(school_id, request: Request):
    ...
    data = school_models.APISchoolData.from_airtable_school(
        airtable_school=airtable_school, url_path_for=request.app.url_path_for
    )

    return school_models.APISchoolResponse(
        data=data, links={"self": request.app.url_path_for("get_school", school_id=school_id)}
    )
...
async def get_school_hub(school_id, request: Request):
    ...
    return hub_models.APIHubResponse(
        data=data, links={"self": request.app.url_path_for("get_school_hub", school_id=school_id)}
    )
